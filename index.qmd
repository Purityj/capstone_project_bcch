---
title: "Evaluating the Impact of CPOE on Pediatric Laboratory Services at BC Children's Hospital"
...
Several limitations affect interpretation. First, this is an observational study — changes observed after CST should not be automatically attributed to CST alone, as workflow changes, post-COVID recovery, patient mix, staffing, and location-specific practices may all contribute. Second, some fields are workflow-dependent and may be incomplete for the pre-CST period. Third, accession numbers do not perfectly represent individual blood draws, as deriving collection events requires assumptions. Fourth, Clinical Collect status cannot be fully verified — while device and collector combinations and timing are visible, manual edits to collection times are not. Fifth, location names and workflows may change over time, requiring crosswalks or partner clarification. Finally, mislabeling may not be captured by cancellation fields alone and may require text mining of result descriptions.

## Deliverables and Success Criteria

Deliverables include a reproducible analysis pipeline, documented derived variables, validated summary tables and visualizations, and a dashboard prototype for monitoring CPOE performance over time.

Success is defined by producing clear, defensible answers to the provided research questions using appropriate methods and validated data transformations. This requires a reproducible workflow, clearly stated assumptions and limitations, and results that are statistically sound and practically meaningful in the hospital context. The dashboard should enable stakeholders to monitor CPOE performance, identify workflow changes, and assess whether CPOE is achieving its intended benefits.

## Timeline

The project spans six weekly phases, moving from data exploration and scoping through iterative analysis, modeling, and visualization, to a final dashboard prototype, partner validation, and handoff.
